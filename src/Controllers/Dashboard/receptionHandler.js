...
const oldAddReception = async (req, res, next) => {
  try {
  ...
  try {
    ...
    if (services && services.length >= 1) {
      ...
      if (discountCode) {
        ...
        if (discount) {
          selectedDiscount = await customerDiscountsModel.findOne({
            where: {
              customerId,
      ...
      if (reception.dataValues.id) {
        if (payments && payments.length >= 1) {
          ...
          await Promise.all(payments.map(async (payment) => {
            await receptionPaymentModel.create({
...
const editReception = async (req, res, next) => {
  try {
    ...
    const isNewUser = await receptionModel.findOne({
      where: {
        ID: id,
      },
    });
    if (isNewUser !== null) {
      const data = await receptionModel.update(
        {
          USER_ID: userId,
          PRICE: price,
          DESCRIPTION: description.length ? description : null,
...
const deleteReception = async (req, res, next) => {
  try {
    const { id } = req.params;
    if (id) {
      const rowCount = await receptionModel.update(
        {
...
const getReception = async (req, res, next) => {
  try {
    ...
    let query =
      'SELECT mariNail_Receptions.id, mariNail_Receptions.PRICE, mariNail_Receptions.DESCRIPTION, mariNail_Receptions.CREATED_AT from mariNail_Receptions WHERE (mariNail_Receptions.DELETED_AT = 0) ORDER BY CREATED_AT DESC';
    let reception;
    ...
    reception = await db.query(query, { type: QueryTypes.SELECT });
    res.send({
      status: 1,
...
const getSpecificReception = async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    const [reception] = await db.query(getReceptionQuery, { type: QueryTypes.SELECT });
    ...
    const [customer] = await db.query(getCustomerDetail, { type: QueryTypes.SELECT });
    ...
    const receptionService = await db.query(getReceptionService, { type: QueryTypes.SELECT });
    ...
    const subServices = await db.query(getSubService, { type: QueryTypes.SELECT });
    res.send({
      status: 1,
